    ...
    for idx, type_dir in enumerate(gt_and_prediect) :
        ...
        for file in tqdm(os.listdir(type_dir)):
            ...
            if type_dir == labelPath :
                ...
                # YOLO Predict(XYWH to XYXY)
                for labelinfo in labelinfos:
                    cls, x_center, y_center, width, height, conf = map(float, labelinfo.strip().split())
                    rx1, ry1 = x_center - width / 2, y_center - height / 2
                    rx2, ry2 = x_center + width / 2, y_center + height / 2
    ...
    for labelfile in labelfiles:
        ...
        for x1, y1, x2, y2, conf in rectinfos:
            ...
            if conf == 1.0:
                ...
                cv.rectangle(img, (x1, y1), (x2, y2), rectcolor, 4)
                text = 'detect'
                cv2.putText(img, text, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 1.5, rectcolor, thickness=2)
...
image_path = "/root/dataset_clp/dataset_2044_new/test/images"
label_path = "/root/dataset_clp/dataset_2044_new/test/labels"
predict_path = "/usr/src/ultralytics/runs/detect/val7/labels"
save_path = "/root/De-identification-CLP/ensemble_model/test_yolov8_gt"

detections, groundtruths, classes = boundingBoxes(labelPath=label_path, 
...
boxPlot(detections + groundtruths, image_path, savePath=save_path)
